...
- **Termination Criteria**:
  - `--essential`: Stop when Clarity Score > 65.
  - `--standard`: Stop when Clarity Score > 80.
  - `--deep`: Stop when Clarity Score > 95 or user sends `$intent-done`.
...
1. **[Initialize]**: **CRITICAL**: Always check for `.omg/state/interview-context.json` using `read_file`. 
   ...
   - **If the file is missing**: **Request the user to start** a new interview session by running `/omg:intent --essential|--standard|--deep [query]`.
