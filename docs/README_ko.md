...
## Gemini CLI 호환성 노트 (검토일: 2026-04-16)

- 권장 최소 검증 baseline: Gemini CLI `v0.37.0+`
...
OmG는 Gemini CLI가 모델 요청을 보내기 직전에 현재 모델 정책이 눈에 띄게 보이도록 `BeforeModel` 배너도 기본 포함합니다.

- 훅 엔트리포인트: `hooks/hooks.json` (`BeforeModel` -> `omg-before-model-banner`)
- 스크립트: `hooks/scripts/before-model-banner.js`
- 표시 내용: 가능하면 요청 예정 런타임 모델, 현재 OmG 모델 전략, lane alias, workspace `general.previewFeatures` 상태
- 기본 배너 예시:
...
- 이 배너만 끄려면 `OMG_DISABLED_HOOKS=model-preview` 또는 `model-banner`
- `/omg:status`와 HUD 미리보기에도 같은 모델 전략 요약을 더 눈에 띄게 표시합니다.
