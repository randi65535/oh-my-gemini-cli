...
- tmux를 이용한 백그라운드 병렬처리 기능을 추가했습니다:
  - `/omg:ultrawork --tmux` 플래그를 사용하면 각 shard를 별도의 tmux 윈도우에서 백그라운드 Gemini CLI 프로세스로 실행합니다.
  - 새로운 `$parallel` deep-work 스킬이 독립 tmux shard의 분해, 실행, 모니터링, 결과 수집 프로토콜을 제공합니다.
  - 새로운 `scripts/tmux-parallel.js` 헬퍼 유틸리티가 tmux 세션 수명 주기(create / send / status / capture / kill / list)를 관리하고 세션 메타데이터를 `.omg/state/parallel.json`에 저장합니다.
  - `context/omg-core.md` 업데이트: `$parallel` 스킬이 retained deep-work 스킬 목록에 추가되었고, Parallel Rule이 `--tmux` 모드를 명시적으로 참조합니다.
...
독립 shard를 별도의 tmux 윈도우에서 백그라운드 Gemini CLI 프로세스로 실행하려면 `/omg:ultrawork --tmux`를 사용하세요. 메인 세션은 shard가 실행되는 동안 다른 작업을 계속할 수 있습니다.

**전제 조건:** 호스트에 `tmux`와 `gemini` CLI가 설치되어 있어야 합니다.

**적합한 상황:**
- 여러 독립 주제를 동시에 조사할 때
- 겹치지 않는 모듈/파일을 병렬로 생성하거나 분석할 때
- shard 간 공유 파일 접근이 없는 대규모 백로그를 처리할 때

**빠른 예시:**

```text
/omg:ultrawork --tmux "현재 코드베이스의 보안, 성능, 접근성을 분석해"
```

OmG는 다음을 수행합니다:
1. 3개의 독립 shard로 분해 (보안 / 성능 / 접근성)
2. 3개 윈도우를 가진 tmux 세션 `omg-par-<id>` 생성
3. 각 윈도우에서 `gemini -p "<shard_prompt>" > shard-<n>.output.md` 실행
4. 세션 메타데이터를 `.omg/state/parallel.json`에 기록
5. 완료 마커(`done.log`의 `SHARD_DONE_<n>`) 확인
6. 모든 shard 완료 후 결과를 수집해 `merged.md`에 합성

세밀한 제어가 필요하면 `$parallel` 스킬을 사용하세요:

```text
$parallel
```

`scripts/tmux-parallel.js` 헬퍼는 직접 tmux 세션 관리(create / send / status / capture / kill / list)를 위해 제공됩니다.

**안전 규칙:**
- 동시에 활성화할 수 있는 tmux 세션은 최대 4개입니다.
- 같은 파일 경로나 git 브랜치에 쓰는 shard는 절대 병렬 실행하지 마세요.
- 모든 shard 출력은 `.omg/state/parallel/<session-id>/` 아래에 저장됩니다.
...
| `/omg:ultrawork` | 독립 작업 배치 처리 중심 고처리량 모드; `--tmux` 플래그를 추가하면 별도의 tmux 윈도우에서 백그라운드 병렬 실행 | 대규모 백로그 또는 진정한 백그라운드 병렬처리가 필요할 때 |
...
| `$parallel` | 독립 작업을 tmux 기반 백그라운드 Gemini CLI shard로 분해하고 결과를 수집·합성 | 세션 맵, shard 계획, 합산 결과 요약 |
...
| `--tmux`로 시작한 shard가 완료되지 않음 | shard 프롬프트가 같은 파일에 쓰거나 tmux 세션이 종료됨 | `.omg/state/parallel/<id>/done.log`에서 `SHARD_DONE_<n>` 마커 확인; `node scripts/tmux-parallel.js status <id>` 실행 또는 `node scripts/tmux-parallel.js kill <id>`로 세션 정리 후 재실행 |
| `--tmux` 사용 시 `tmux: command not found` | 호스트에 tmux가 설치되지 않음 | tmux 설치 (`brew install tmux`, `apt install tmux` 등) 후 재실행 |
