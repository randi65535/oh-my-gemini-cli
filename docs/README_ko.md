...
- 최근 `oh-my-codex` 변경 이력 검토를 바탕으로 baseline-aware lane guardrail을 추가했습니다.
  - `commands/omg/workspace.toml`이 lane별 baseline branch/HEAD anchor를 기록할 수 있습니다.
  - `commands/omg/taskboard.toml`, `commands/omg/team-plan.toml`이 task slice에 baseline 메타데이터를 유지합니다.
  - `commands/omg/team-prd.toml`, `commands/omg/team-exec.toml`, `commands/omg/team.toml`이 baseline 기대값을 실행 handoff 안전 계약에 포함합니다.
- 드리프트 감지와 운영 가시성을 강화했습니다.
  - `commands/omg/team-exec.toml`은 active lane이 기록된 baseline에서 작업 의미가 바뀔 정도로 벗어난 경우 슬라이스를 `blocked`로 멈춥니다.
  - `commands/omg/status.toml`은 baseline-anchor drift 위험을 드러냅니다.
  - `commands/omg/doctor.toml`은 baseline anchor 누락/드리프트를 진단합니다.
  - `context/omg-core.md`에 baseline integrity를 핵심 실행 규칙으로 반영했습니다.
- 패키지/확장 버전을 `0.7.4`로 올리고 README/한국어 README/랜딩/히스토리를 갱신했습니다.
