...
- `/omg:model` 명령을 추가해 OmG 기본 모델 선택 전략을 조회/설정할 수 있게 했습니다.
- 전략 프리셋을 추가했습니다.
  - `balanced`: lane별 기본 분배 (`gemini-3.1-pro-preview`, `gemini-3-flash-preview`, `gemini-3.1-flash-lite-preview`)
  - `auto`: 모든 lane 모델 선택을 Gemini 런타임의 auto 모델 정책에 위임
  - `custom`: planning/execution/quick/review-verify lane별 모델을 직접 지정
- 모델 정책을 `.omg/state/model.json`에 저장해 장기 세션에서도 동일 전략을 재사용할 수 있습니다.
- OmG 오케스트레이션 전략과 Gemini 런타임 모델 제어(`/model`, 지원 시 시작 옵션 `--model`)의 경계를 문서화했습니다.
- 패키지/확장 버전을 `0.7.0`으로 올리고 README/한국어 README/랜딩/히스토리를 갱신했습니다.
## 한눈에 보기
