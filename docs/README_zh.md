...
- 将访谈状态从单一共享文件改为按需求线程分开的会话目录：
  - `/omg:intent` 与 `/omg:interview` 现在面向 `.omg/state/interviews/[slug]/context.json`
  - `.omg/state/interviews/active.json` 用作当前活跃访谈的确定性指针
- 对齐了新访谈会话布局下的命令契约、代理契约与核心工作流上下文：
  - `commands/omg/intent.toml`
  - `commands/omg/interview.toml`
  - `agents/interview.md`
  - `context/omg-core.md`
- 扩展/包版本提升至 `0.7.8`，并刷新 README、韩文 README、中文 README、落地页文档与历史记录

## 访谈会话存储

- `/omg:interview` 的会话状态现在建议存放在 `.omg/state/interviews/[slug]/` 下，而不是一个共享访谈文件中。
- `.omg/state/interviews/active.json` 记录当前访谈，使 resume/status 流程在多个需求线程之间保持确定性。
- 这样同一项目中的多次需求澄清就能分开保存，也更便于归档。
