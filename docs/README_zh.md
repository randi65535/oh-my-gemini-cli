...
| 默认模型策略 | 判断/验收门控用 `gemini-3.1-pro`，重实现任务用 `gemini-3-flash`，广覆盖低风险探索用 `gemini-3.1-flash-lite` |
...
| 代理 | 主要职责 | 推荐模型配置 |
| --- | --- | --- |
| `omg-architect` | 系统边界、接口、长期可维护性 | `gemini-3.1-pro` |
| `omg-planner` | 任务拆解与顺序规划 | `gemini-3.1-pro` |
| `omg-product` | 范围锁定、非目标定义、可度量验收标准 | `gemini-3.1-pro` |
| `omg-executor` | 快速实现循环 | `gemini-3-flash` |
| `omg-reviewer` | 正确性与回归风险审查 | `gemini-3.1-pro` |
| `omg-verifier` | 验收证据门控与发布就绪性检查 | `gemini-3.1-pro` |
| `omg-debugger` | 根因分析与补丁策略 | `gemini-3.1-pro` |
| `omg-consensus` | 方案评分与决策收敛 | `gemini-3.1-pro` |
| `omg-researcher` | 外部方案分析与综合 | `gemini-3.1-pro` |
| `omg-director` | 团队消息路由、冲突协调、生命周期编排 | `gemini-3.1-pro` |
| `omg-consultant` | 战略分析标准与建议框架 | `gemini-3.1-pro` |
| `omg-editor` | 最终交付结构、一致性与受众匹配 | `gemini-3-flash` |
| `omg-quick` | 小型战术修复 | `gemini-3.1-flash-lite` |
