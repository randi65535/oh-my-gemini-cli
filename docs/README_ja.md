...
| 既定モデル戦略 | 判断/受け入れゲートは `gemini-3.1-pro`、実装中心作業は `gemini-3-flash`、広範な低リスク探索は `gemini-3.1-flash-lite` |
...
| `omg-planner` | タスク分解と順序設計 | `gemini-3.1-pro` |
| `omg-product` | スコープ固定、non-goal、計測可能受け入れ条件 | `gemini-3.1-pro` |
| `omg-executor` | 高速実装サイクル | `gemini-3-flash` |
| `omg-reviewer` | 正確性と回帰リスクの確認 | `gemini-3.1-pro` |
| `omg-verifier` | 受け入れゲート証拠と release readiness 検証 | `gemini-3.1-pro` |
| `omg-debugger` | 根本原因分析とパッチ戦略 | `gemini-3.1-pro` |
| `omg-consensus` | オプション評価と意思決定収束 | `gemini-3.1-pro` |
| `omg-researcher` | 外部選択肢の分析と統合 | `gemini-3.1-pro` |
| `omg-director` | チーム内メッセージルーティング、衝突解消、ライフサイクル統括 | `gemini-3.1-pro` |
| `omg-consultant` | 戦略分析基準と提案フレーミング | `gemini-3.1-pro` |
| `omg-editor` | 最終成果物の構造、一貫性、対象読者適合 | `gemini-3-flash` |
| `omg-quick` | 小規模な戦術修正 | `gemini-3.1-flash-lite` |
