...
- On non-trivial work: `intent` -> `workspace` when multiple roots or parallel lanes matter -> `team-assemble` when role fit is unclear -> `team-plan` -> `team-prd` -> `taskboard` -> `team-exec` -> `team-verify` -> `team-fix`.
...
- Prefer stable task IDs, workspace lane labels, and compact evidence pointers over repeating raw diffs in chat.
...
- Never mark work done when open taskboard items or missing verifier signoff remain.
